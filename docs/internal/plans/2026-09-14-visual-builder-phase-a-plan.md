...
- **A control never competes with a legacy field.** A block type's Style controls are enabled only when its `flags.legacy_presentation` is false, and that flag is cleared in the same slice that removes the block's legacy presentation fields and converts them.
...
**Files:**
- Modify: `ApplyPreviewData.php` (+ `base_revision: ?int`, `operations: ?list<array>` (the committed transaction's sanitised ops; used for invalidation intent in A6, validated against both documents), no `changed` field), create `ApplyPreviewResultData.php` (`revision`, `baseline`, `style_generation`, `applied_at`), `PreviewWorkingCopyStore.php` (a per entry+locale record `{revision, fields, ops, accepted_at}` in the cache with a companion lock key; acceptance is compare-and-set: the request's `base_revision` must equal the stored revision exactly, applied under the lock, else 409 `PREVIEW_REVISION_STALE` with the current revision; a null `base_revision` initialises only when no record exists; save clears the record and the next apply re-initialises; expiry behaves like save), create `core/src/Content/Style/SiteStyleGeneration.php` (system flag `style.generation`, incremented in Phase B), `EntryController::applyPreview()`, `PreviewController` mint response (+ current `revision` so a second editor initialises from accepted state), `RenderController::preview()` (canvas mode embeds `data-thallo-revision` on `<main>` from the working copy), `preview-bridge.js` (`stage-refresh` answers `stage-refreshed {revision}` read from the fetched page, never echoed from the request; a fetched revision older than displayed is refused), `useCanvasBridge.ts`, `[locale].vue` (local, accepted, displayed; §3.5 table; undo/redo buttons and ⌘Z/⇧⌘Z; every mutation through ops)
- Test: `PreviewRevisionTest.php` (two simultaneous same-base applies: one wins with r+1, the other 409; save then apply re-initialises; expiry then apply; a second editor minting gets the accepted revision; a refresh racing another editor's apply displays the newer revision), `canvas-revisions.spec.ts` (every table row; save of revision 10 completing after an edit to 11 leaves 11 dirty; stale response dropped; baseline mismatch refreshes), `canvas-page.spec.ts` (undo reverts a stage move)
...
**Files:**
- Create: `core/src/Content/Style/Conversion/ConversionTable.php` (§7.2 as data per field: `convert(closure)`, `keep`, `unmappable`), `Converter.php`, `DiagnosticsReport.php` (JSON lines with a source-generic identity: `source_type`, `source_id`, `source_revision`, `locale`, `block_id`, `field`, `old_value`, `status` in `converted|kept|unmappable|superseded|discarded`, `reason`, `document_hash`, `converter_version`), `DecisionsFile.php` (keyed by `source_type:source_id:source_revision:block_id:field`, storing `document_hash`, `converter_version`, `action`, `value`; a decision is invalid when either hash or version differs), `ConvertSettingsCommand.php` (`thallo:blocks:convert-settings --dry-run --report=path`, `--decisions=path`; refuses live while any diagnostic is unresolved; each document is converted and persisted atomically with `_schema.settings = 1` written in the same persistence operation; idempotent by stamp; runs under the migration gate)
...
- Test: `ConverterTest.php` (button shape → radius; heading align → alignment.text; a `17px` padding → unmappable; a decision converts it; a stale decision by hash or by converter version is rejected; live refuses with one unresolved; stamp makes a rerun a no-op; regions and old versions convert; `heading.align = left` with `settings.alignment.text.base = center` keeps center and reports superseded; a crash injected between convert and persist leaves the document unstamped and unconverted), `ConvertSettingsCommandTest`
...
### Task A5.3: slice gates and release

- [ ] Rehearsal scenarios extended with group two; full gates; `CHANGELOG.md`; beta on the user's word. thallo.dev: re-author the homepage entirely from the inspector.
...
**Release-slice check:** A1 changes storage only, additively. A2 changes delivery with identical output and declares capabilities without controls. A3 changes the editor protocol with no new values written. A4 removes fields only for the block types whose controls it ships, gated by `legacy_presentation`. A5 removes the rest and activates the inline-style lint only once nothing emits one. A6 is off.
