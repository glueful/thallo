...
- `layout.spec.js`, **the surviving `spacing-flex-then-reset-md`**: it expects block-flow margins to come back after the reset ("Reset is the same state as block … so the margins return identically"). A reset now lands on the theme default, which is a flex column: the expectation becomes no child margin at any width, with the children `--space-5` apart from `md` up by the default gap and by the authored gap below it. The expectations for the two deleted cases leave the table with them.
