...
- [x] **Ecommerce content integration** — shipped 2026-07-21 — the `glueful/thallo-commerce` pack: per-workspace adoption of `glueful/commerce`, the product ↔ entry link (`ProductLinkService`, the `product-story` starter content type) and the themed storefront. Commit `54cd3a1b`; see `packages/thallo-commerce/README.md`.
- [x] **Multi-workspace setup: admin-settable resolution hosts** — shipped 2026-07-14 — the base domain and default hosts are persisted admin settings (`PUT /v1/admin/tenancy/public-origin`, the Public origin panel in Settings › Workspaces, `Thallo\Tenancy\PublicOrigin\PublicOriginService`), so activating full resolution needs no `.env` edit; the panel shows when a restart is still needed to apply a change. Commit `fe6e03ef`.
...
- [x] **Form block** — shipped 2026-07-09 — generic `form` block (contact preset): sealed-descriptor model, stored + emailed submissions (the default `NotificationFormMailSender` sends through the notification service's email channel, configured in Settings › Email; an "email only" form stores the submission when the mail fails), spam guard chain, admin Submissions area + CSV, delivery mode (store+email / email-only), selectable submit-button style. [spec](superpowers/specs/2026-07-09-form-block-design.md) · [plan](superpowers/plans/2026-07-09-form-block.md).
...
- [x] **Capability packs** — seo, collections, search, analytics, workflow, navigation, importers — built as capability packs on the composable-core seams (switched at runtime in Extensions › Capabilities).
