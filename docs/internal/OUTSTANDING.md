...
- [ ] **Subscriptions: workspace self-serve checkout** — `M` — provider session origination so a workspace can self-serve upgrade/manage its own billing, instead of today's admin-only Plans/Billing surface. Deliberately deferred as a dedicated follow-up product decision, not part of Phase 2's scope. **Home:** [thallo-subscriptions spec §8](superpowers/specs/2026-08-03-thallo-subscriptions-design.md) — follow-up on shipped Subscriptions.
- [ ] **Subscriptions: pricing blocks ↔ platform plans bridge** — `S` — optional `plan_key` field on the `pricing_plan` block plus a capability/engine-gated hydrator: defaults (plan name, provider-derived price if surfaced) filled from the plans catalog with author overrides winning, CTA generating the checkout URL. Engine off ⇒ block renders authored content exactly as today — the render pack stays standalone. Deliberately bundled with the checkout work (a live plan card needs a checkout destination; today the engine stores no display copy to sync). **Home:** _(design with checkout)_ — follow-up on shipped Subscriptions + render pricing blocks.
