...
- [ ] **Framework: extension providers resolve `MigrationManager` at boot — eager connect + DDL survives 1.78.3** — `S` — FIX LANDED on framework dev (`ccc70f5`, ships in 1.78.4 — awaiting publication): `MigrationManager` now follows the lazy-ledger contract (spec `docs/internal/superpowers/specs/2026-08-17-schema-creation-policy-design.md` A1) — construction resolves no connection, `migrate()` is the only ledger creator, status/pending/rollback treat a missing ledger as zero applied with no DDL. Verified on the published beta.2 artifact with the patched file: `php glueful list` and `migrate:status` create nothing on an empty database, and a Postgres statement-log spy shows boot issues zero writes of any kind. Tick + move to Recently shipped when 1.78.4 publishes and Thallo repins.
