...
- [ ] **Framework: extension providers resolve `MigrationManager` at boot — eager connect + DDL survives 1.78.3** — `S` — 1.78.3 made the three `Migrate/*` commands lazy, but `Extensions/ServiceProvider::loadMigrationsFrom()` still does `$this->app->get(MigrationManager::class)` during provider boot, and the constructor connects AND runs `ensureVersionTable()`: on any install with a reachable database, `php glueful list` alone creates the `migrations` table (as whatever role `.env` names — the exact ownership trap the beta.1 gate hit). Unreachable DB degrades gracefully, so this is no longer launch-blocking, but boot-time DDL remains. Candidate: register migration paths as data at boot and resolve the manager only when a migrate operation actually runs. Surfaced by the beta.2 artifact gate. **Home:** _(no design yet)_ — framework follow-up.
- [ ] **Provision: non-interactive mode rejects passwordless (trust/peer) PostgreSQL and unset host** — `S` — `thallo:provision -n` hard-requires a non-empty `DB_PGSQL_PASSWORD` and an explicit host, so the common local trust-auth setup (Homebrew/Postgres.app, socket or localhost, no password) cannot pass validation at all — the beta.2 gate had to create a password-bearing role just to proceed. Candidate: default host to `localhost`, allow an explicit empty password (e.g. `--db-password=""` meaning "none"), and let the preflight connection attempt be the arbiter. Surfaced by the beta.2 artifact gate. **Home:** _(no design yet)_.
- [ ] **Provision/tenancy: a run that fails argument validation still mutates schema** — `S` — `thallo:provision -n` with missing db settings refuses with an error, but by then boot has already created ~18 tenancy/content tables (`block_types`, `block_type_migrations`, `entries`, `entry_*`, `content_types`, … — `ThalloTenantTables` path): schema work runs during application boot, before the command validates its input, leaving a half-initialized database behind a "nothing happened" error message. Candidate: defer tenant-table bootstrap out of cold boot (or behind the same laziness as the framework item above). Surfaced by the beta.2 artifact gate. **Home:** _(no design yet)_.
