...
**Goal:** Ship glueful/subscriptions **2.1.0** (three additive seams + one spec-wording amendment) and then the **thallo-subscriptions** capability pack — workspace SaaS billing in Thallo per `docs/internal/superpowers/specs/2026-08-03-thallo-subscriptions-design.md`.

**Architecture:** Two repos in strict order. Phase A (subscriptions repo, Tasks 1–4): `currentForTenants()` trusted bulk read, `countSubjectRows()` purge seam, `SubscriptionSchemaReadiness` authority, spec amendment, release 2.1.0. Phase B (Thallo repo, Tasks 5–12): pack scaffold + wiring, host subject resolver, lazy `EngineGateway`, plans + workspaces admin APIs (platform authority, provider-managed guard), fail-closed purge integration, SPA module, and the capability/engine truth table.
...
- `currentForTenants()` is a trusted administrative projection: no per-UUID `validate()`; contract requires a normalized, deduplicated, host-directory-derived list after platform authorization; never mounted as a batch-by-UUID HTTP input (spec §6.1). Thallo proves the precondition: UUID list only from `TenantAdministration::listTenants()`, no caller-supplied UUID filter (spec §3).
...
- Controllers never constructor-inject engine services; `EngineGateway` probes per-operation; meta is always 200 with `engine_disabled | schema_not_ready | ready`; readiness comes ONLY from the upstream `SubscriptionSchemaReadiness` authority — a lone legacy table or partial 006 is never `ready` (spec §5, §6.3).
...
- Resolver ruling (spec §2): `currentTenant()` = tenancy context else `SingleStoreTenant::resolve()`; `validate()` = tenant self-subject + `TenantAdministration::getTenant()` existence; user subjects always false; `currentUser()` null.
...
**Interfaces:**
- Produces: `SubscriptionRepository::findTenantSubjectsAmong(ApplicationContext $c, array $tenantUuids): array` — ONE `whereIn` query over `subject_type='tenant' AND tenant_uuid IN (...)`, rows decoded like `findBySubject()`. `SubscriptionService::currentForTenants(array $tenantUuids): array` — normalizes (string-cast, drop empties, dedupe), returns `[tenantUuid => row]` (absent key = no subscription). Docblock carries the §6.1 trusted-projection contract verbatim (no per-UUID validate; host-directory-derived list; never a batch-by-UUID HTTP input).
...
public function currentForTenants(array $tenantUuids): array
{
    ...
    $out = [];
    foreach ($this->subscriptions->findTenantSubjectsAmong($this->context, array_keys($normalized)) as $row) {
...
### Task 2: `countSubjectRows()` purge seam
...
- [ ] **Step 1: Failing tests:** counts match what a subsequent `purgeSubject()` deletes (capture counts, purge, assert per-table deleted == counted); all-zero after purge; idempotent (second count identical); user-subject form never counts plans; sibling users/foreign workspaces uncounted; `assertPurgeableSubject` guard applies (empty ids throw).
- [ ] **Step 2:** RED. **Step 3:** Extract the WHERE-builders from `purgeSubject()` into private methods; implement counts as `SELECT COUNT(*)` per table via the shared builders, wrapped in `TenantIntegration::runAsSystemOr` like `purgeSubject()`.
- [ ] **Step 4:** GREEN; full suite + gates. **Step 5:** Commit `feat: non-mutating countSubjectRows purge seam`.
...
- Test: `tests/Integration/Schema/SubscriptionSchemaReadinessTest.php` (new; uses `LegacySchemaTestCase` for the 1.x fixture and the shared harness for 2.x)

**Interfaces:**
- Produces: `final class SubscriptionSchemaReadiness { public function __construct(private readonly ApplicationContext $context) {} public function isReady(): bool; }` — true ONLY when the complete minimum 2.x runtime shape exists: tables `subscriptions`, `subscription_overrides`, `subscription_events`, `subscription_plans`, `subscription_provider_event_receipts`; columns `subscriptions.subject_type/subject_uuid/plan_uuid`, `subscription_plans.audience/owner_tenant_uuid`. Uses the framework schema builder's `hasTable`/`hasColumn`; every probe wrapped so a thrown DB error returns false (readiness is a probe, never a fatal). Registered `shared => true` in `services()` + a `ServiceProviderWiringTest` row.

- [ ] **Step 1: Failing tests:** fresh 2.x harness ⇒ true; empty database (drop all) ⇒ false; legacy 1.x schema (`LegacySchemaTestCase`, migrations 001–004 only) ⇒ false; partial shapes ⇒ false — (a) 006's columns absent but receipts table present is unreachable in practice, so pin the two realistic partials: base tables + receipts table missing; base tables + `plan_uuid` column missing; DB-throw path (probe against a closed/broken connection double) ⇒ false.
...
**Files:** `docs/superpowers/specs/2026-08-02-subscriptions-v2-subject-model-design.md` (§4: the "binding the resolver is enabling memberships" sentence → "memberships are enabled only when the host resolver positively resolves AND validates user subjects; a tenant-only host resolver binds without enabling them"), `CHANGELOG.md` (2.1.0: three seams, additive, no behavior changes for existing callers; the trusted-projection contract summarized), `composer.json` (`extra.glueful.version` → `2.1.0`), README (short "Bulk administrative reads" + "Schema readiness" notes), version-pin test update.

- [ ] Edits; full gates; commit per release convention (`Release 2.1.0 — host integration seams`); local tag `v2.1.0`; NOTHING pushed (maintainer publishes — Phase B Task 5 needs the published release or the path-repo fallback).
...
- [ ] **Step 1:** `composer update glueful/subscriptions glueful/thallo-subscriptions` (2.1.0 must resolve — from Packagist if published, else add the sibling path repo with `canonical:false` exactly as the subscriptions repo does for contracts, and note it in the report). Run `php glueful migrate:run` so the engine's tables exist in the dev install.
...
**Interfaces:**
- Produces: `final class ThalloSubjectResolver implements SubjectResolverInterface` — ctor `(ApplicationContext $context, SingleStoreTenant $singleStore, TenantAdministration $tenants)` with `TenantAdministration` soft-resolved (nullable) since the contract binding may be absent pre-tenancy-install; `currentTenant()`: tenancy context when enforcement on (reuse the same probe `CurrentTenant`-style via request state/TenantContext), else `SingleStoreTenant::resolve()`; `validate()`: `$s->type === SubjectType::TENANT && $s->uuid === $s->tenantUuid && $s->tenantUuid !== ''` AND (when `TenantAdministration` bound) `getTenant($context, $s->tenantUuid) !== null`, with the documented fallback that an unbound administration contract accepts coherent self-subjects (single-store pre-tenancy posture — the default workspace exists by construction); user subjects `return false`; `currentUser()` `return null`.

- [ ] **Step 1: Failing tests:** tenancy-off resolution returns the default workspace uuid (seed via `SystemFlags`/SingleStoreTenant harness idioms — copy the setup from thallo-tenancy's own `SingleStoreTenant` tests); coherent tenant self-subject with an existing tenant ⇒ true; nonexistent tenant uuid ⇒ false; `subject_uuid !== tenant_uuid` ⇒ false; empty ids ⇒ false; `Subject::user(...)` ⇒ false always; `currentUser()` null; the container binding resolves THIS class (not the engine default) — assert `get(SubjectResolverInterface::class) instanceof ThalloSubjectResolver`.
...
**Interfaces:**
- Produces routes:
  - `GET /meta` → 200 always: `{engine: <state>, tenancy_enabled: bool, default_tenant_uuid: ?string}` (uuid only when tenancy off; from `SingleStoreTenant::resolve()`)
  - `GET /workspaces` → tenancy ON: page through `TenantAdministration::listTenants()`, join via `currentForTenants(array_column($tenants, 'uuid'))`; tenancy OFF: single row for the default workspace. Response rows: `{tenant: {...directory fields}, subscription: null | {status, plan_key, plan_display_name, trial_ends_at, grace_ends_at, provider_managed: bool}}`. `plan_display_name` resolved via ONE `PlanCatalog` platform-scope lookup per distinct plan_key in the page (small in-memory map — not per-row queries). NO caller-supplied UUID filter parameter exists (spec §3).
  - `GET /workspaces/{uuid}` → detail incl. overrides list (`activeForSubject(Subject::tenant($uuid))`)
  ...
  - `PUT /workspaces/{uuid}/overrides/{entitlement}` body `{value, expires_at?, reason?}` → `upsertForSubject`
  - `DELETE /workspaces/{uuid}/overrides/{entitlement}` → `deleteForSubject`
  All workspace routes validate `{uuid}` against `TenantAdministration::getTenant()` (404 when unknown; tenancy-off: only the default workspace uuid is valid). Same middleware + trait as Task 8.

- [ ] **Step 1: Failing tests:** meta in all three engine states (200 each, correct payload) and both tenancy modes; workspace index against a multi-tenant fixture — asserts join correctness, absent-subscription rows, `provider_managed` flag, a repository-call spy proving ONE bulk read per page, and that a `?uuids=` query param is IGNORED (404-free, unfiltered result — the no-caller-input pin); detail + overrides round-trip (upsert → visible in detail → delete → gone), honored by the tenant entitlement resolver (one end-to-end `allows()` check); set-plan on fresh workspace (start) and existing (changePlan); provider-linked fixture: PUT plan and POST cancel both 409 `provider_managed_subscription`, manual fixture both succeed; unknown workspace 404; 403 posture loop as Task 8.
...
**Interfaces:**
- Produces: `final class SubscriptionsPurgeHandler implements \Thallo\Tenancy\Purge\PurgeHandler` — `id(): 'subscriptions'`; `dependsOn(): []`; ctor `(ApplicationContext $context, EngineGateway $gateway)`. Behavior matrix (spec §5): schema absent (readiness probe says no subscriptions table — use `SubscriptionSchemaReadiness` when bound, else a direct `hasTable('subscriptions')` probe when the engine is fully absent) ⇒ `prepare` returns zero-artifact, `verify` true; schema EXISTS but `$gateway->purger()` null ⇒ `prepare`/`purge`/`verify` ALL throw `\RuntimeException('subscriptions data exists but the subscriptions engine is disabled — enable the extension before purging this tenant')`; engine ready ⇒ `prepare` = `countSubjectRows(Subject::tenant($uuid))` artifact, `purge` = `purgeSubject(...)`, `verify` = all counts zero.

- [ ] **Step 1: Failing tests:** the three-row matrix above (schema-absent pass; schema-present-engine-disabled throws from all three methods; ready path counts→purges→verifies with seeded billing data incl. member plans owned by the workspace); alias registered EVEN when the capability is disabled (flip `thallo.subscriptions` off in the harness config) and even when the engine provider is absent; tenancy registry test now lists `subscriptions` among handler ids.
...
- Behavior: both pages fetch `/meta` first; `engine_disabled` renders `EngineStateNotice` with a link to `/extensions`; `schema_not_ready` renders the run-migrations notice; Billing renders the directory (tenancy on) or the "This site's plan" panel (off — same `WorkspaceDrawer` bound to `default_tenant_uuid`); the drawer disables set-plan/cancel with the provider-managed explanation when `provider_managed` is true and renders 409 payload messages verbatim.

- [ ] **Step 1: Failing vitest specs:** module gating (hidden without the capability — reuse the registry test idiom from `commerceModule` coverage); Plans page states (list render, editor submit calls the create endpoint, engine_disabled notice); Billing states (tenancy on directory + drawer actions, tenancy off single panel, provider-managed refusal rendering, all three engine states).
...
- **Spec coverage:** §1 → Task 5 (scaffold, extensions.php, ^2.1, three layers); §2 → Task 6 (+§6.4 wording in Task 4); §3 → Tasks 8–9 (platform authority, bulk join, no-UUID-input, provider-managed 409, meta); §4 → Task 11; §5 → Tasks 7 (gateway/lazy/states), 10 (purge always-alias/fail-closed), route/command posture in Tasks 5+8; §6.1 → Task 1; §6.2 → Task 2; §6.3 → Task 3 (+consumed in Task 7); §6.4 → Task 4; §7 upstream → Tasks 1–3 tests, Thallo → Tasks 6–12 tests incl. the truth table (Task 12) and constant-call-count + UUID-input-exclusion (Tasks 1, 9); §8 respected (no checkout, no memberships, no per-tenant catalogs, no new permission). No gaps.
- **Placeholder scan:** clean — every code step carries real code; the two "mirror the sibling file" instructions name exact files and are pattern-adoption directives, not TBDs.
- **Type consistency:** `currentForTenants(array): array` (Task 1) consumed in Task 9; `countSubjectRows(Subject): array` (Task 2) consumed in Task 10; `SubscriptionSchemaReadiness::isReady(): bool` (Task 3) consumed in Tasks 7 and 10; `EngineGateway` states/accessors (Task 7) consumed in Tasks 8–10; `EngineUnavailableException::$state` naming consistent; alias string `thallo.subscriptions.purge_handler` identical in Tasks 10's two edit sites.
- **Cross-repo sequencing:** Task 5 requires the 2.1.0 release (Packagist) or the documented `canonical:false` path-repo fallback — stated inline.
