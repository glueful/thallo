...
**Modify:** `config/tenancy.php` (add `role_matrix`, `bypass_permissions` — current shape verified: `public_origin` only, `:11-17`), `app/Content/Http/RequirePermission.php`, `app/Providers/CoreServiceProvider.php` (bindings), `packages/thallo-tenancy/routes/enablement.php` (relocation + re-key), `packages/thallo-tenancy/src/Http/Controllers/{TenantDomainController,TenantMembershipController}.php` (target binding), `tests/Integration/Tenancy/RouteCoverageTest.php`.
...
- Modify: `app/Providers/CoreServiceProvider.php` (binding, factory-threaded nullable resolver like `makeTenantBlobPolicy`)
