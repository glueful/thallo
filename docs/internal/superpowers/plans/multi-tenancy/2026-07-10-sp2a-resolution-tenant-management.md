...
- Modify: `app/Providers/CoreServiceProvider.php` (bind both framework seams beside the existing hook bindings)
...
- [ ] **Step 5: Bind both providers in `CoreServiceProvider`; record the media URL audit** — `EngineMediaUrlResolver` has no cache and emits host-relative URLs, so no code change or fictitious `tenant:*:media:*` key is needed there. Run → PASS; phpcs clean. Commit SKIPPED.
