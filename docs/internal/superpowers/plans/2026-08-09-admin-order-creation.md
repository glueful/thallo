...
- Repos/branches: framework and engine work on branches off their `dev` branches; Thallo on `dev`. TWO publication gates: framework v1.77.0 must be published and Thallo repinned BEFORE Task 16's artifact regeneration (its only consumer — Tasks 4-15 do not depend on it; the human may publish it alongside v1.10.0 at the Task 11 gate); Task 11 publishes commerce v1.10.0 and repins Thallo before Tasks 12-16. If Composer cannot resolve either published version at its gate, STOP — no path-repo, vendor-edit, or local workaround, ever. The HUMAN runs each release (push dev, PR dev→main, merge, tag, Packagist). RESEQUENCED 2026-08-09 by human direction: work proceeds past the framework release-prep (commits ready on framework dev) rather than waiting; both publishes may land in one sitting at the Task 11 gate.
