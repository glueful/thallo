...
| Deploy releases from the tag, never `composer update` alone | **Required** | A `create-project` install is Composer's root package: `composer update` moves the framework and the packs but never Thallo. Use `scripts/deploy-site <tag>` (or a fresh `create-project` with `.env`, `storage/` and theme overrides carried across). See [upgrading](upgrading.md). |
| Clear the route table and rendered pages on every deploy | **Required** | `php glueful route:cache:clear && php glueful render:cache:clear` — a stale compiled route table keeps serving the previous release's routes; the compiled container is signed and recompiles itself. `deploy-site` does both. See [upgrading](upgrading.md). |
