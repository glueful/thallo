...
| PHP-served asset paths reach PHP | **Required** | `/theme-assets/*` and `/_thallo/*` (theme runtime, canvas preview support) are served by Thallo, not from disk — only `public/admin/*` and `public/storage/*` are real files. A web-server rule that serves every `.css`/`.js`/`.woff2` URL straight from the document root (CloudPanel's and many nginx templates do) turns them into 404s and the rendered site loads unstyled. Add a location for those two prefixes ABOVE the static-file rule that proxies exactly like `location /`; nginx example: `location ~ ^/(theme-assets\|_thallo)/ { …same proxy_pass/fastcgi as location / … }`. `php glueful thallo:provision` / `thallo:doctor` probe one theme asset on a public `BASE_URL` and warn (`asset-routing`) when it answers 404. |
