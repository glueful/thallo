...
- **Compiled-cache keys carry the version**: `getCacheKey()` returns `db:{theme}:{path}:{version_uuid}`. Every save is a new compiled-cache entry — **no compiled-cache purging exists in this design**. `isFresh()` returns true (versions are immutable).
...
1. **Path validation** (422): same syntactic rules as `asset()` — non-empty, no leading `/`, no `\`, no scheme, no `..` segment — plus **must end `.twig`**. Validation is syntactic only: any safe relative `.twig` path is allowed (hierarchy templates AND partials an override includes). DB-only paths are explicitly allowed — that's the "create in hierarchy" scope; "unknown template" never means "not on the filesystem".
