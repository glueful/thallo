...
| **Code** (`code`) | A code snippet with a language label and a copy button. | `code` (text, required), `language` (enum: text, bash, php, json, yaml, html, css, javascript, typescript, twig, sql), `label` (string), `copy` (boolean) | — | Width, Corners, Shadow |
