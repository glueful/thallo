...
describe('commerce orders query layer', () => {
  ...
  // Invoice data (`GET /commerce/orders/{uuid}/invoice-data`) query/type tests moved to
  // `commerceInvoice.spec.ts` in Task 8 — the implementation now lives in `commerceInvoice.ts`,
  // the one module both the order-detail modal and the printable invoice page import.
})
