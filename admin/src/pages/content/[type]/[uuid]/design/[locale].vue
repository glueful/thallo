...
bridge.onBlockMoveTo((id, neighbor) => {
  ...
  // mutated, so stageStale is untouched and no auto-apply schedules. The drop
  // is judged and generated by the coordinator (visual builder spec §5.1).
  const zone = zoneBeside(id, neighbor)
  if (zone === null) {
    reloadStage()
    return
  }
  coordinator.begin('stage', { blocks: [id] })
  coordinator.propose(zone)
...
/**
 * A neighbour-relative stage intent as a drop zone: the neighbour's list and its index against
 * the tree with the moving block removed (the coordinator's convention).
 */
function zoneBeside(
  id: string,
  neighbor: { beforeId: string } | { afterId: string },
): { parent: string | null; slot: string | null; index: number } | null {
  if (!history) return null
  const neighborId = 'beforeId' in neighbor ? neighbor.beforeId : neighbor.afterId
  const field = history.applier.rootOf(history.document, neighborId)
  if (field === null) return null
  const list = history.applier.rootList(history.document, field)
  const reduced = history.applier.listOps.removeById(list, id)
  const found = history.applier.listOps.locateById(reduced, neighborId)
  if (!found) return null
  return {
    parent: found.parentId,
    slot: found.parentId === null ? field : found.region,
    index: 'beforeId' in neighbor ? found.index : found.index + 1,
  }
}
