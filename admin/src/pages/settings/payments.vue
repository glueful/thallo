...
// tab (`admin/src/pages/commerce/settings/components/PaymentsPanel.vue`, deleted by this task).
// Modeled on `/settings/workspaces` (`admin/src/pages/settings/workspaces/index.vue`): platform
// gating reads `useTenancyAccessStore()` directly rather than a `canManage` prop threaded from
// commerce meta.
//
// Secrets are WRITE-ONLY — the server stores them encrypted and only reports `{set, source}`
// booleans back, so the inputs here never display a stored value: an empty field with a "set"
// hint means "keep the stored value", typing replaces it, and the explicit Clear action sends
// null (the platform row is DELETED — the config/env value, if any, shows through). With no
// gateway extension installed the page honestly reports manual collection.
import { computed, reactive, ref, watch } from 'vue'
import {
  usePlatformPaymentsSettings,
  useSavePlatformPaymentsSettings,
  type PlatformPaymentsSettingsSave,
} from '@/queries/platformPayments'
...
const { data: payments, status } = usePlatformPaymentsSettings(canManage)
const save = useSavePlatformPaymentsSettings()
...
<template>
  <UDashboardPanel id="settings-payments">
    <template #header>
      <UDashboardNavbar title="Payments" />
    </template>
    <template #body>
      <div class="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-6 sm:px-6">
        <!-- Product limitation (platform-payments-settings spec §1.3), rendered prominently and
             unconditionally — it is a standing policy notice, not a load-dependent state. -->
        <UAlert
          color="warning"
          variant="subtle"
          icon="i-lucide-shield-alert"
          title="Every workspace settles through one platform gateway account"
          description="Until Payvia supports explicit merchant connections, EVERY Thallo storefront and SaaS subscription settles through the single platform gateway account. It is not workspace payment isolation."
