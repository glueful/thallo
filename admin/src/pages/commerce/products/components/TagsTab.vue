...
// There is no admin GET for a product's current tag assignments (only the set-list PUT, which
// returns the fresh attached list) — exactly like CategoriesTab's `knownCategories`. `null` =
// never observed this session; `[]` is only reached after a successful set call positively
// established an empty assignment. Never claim "none assigned" for the unknown state, and never
// pre-check boxes from a guess.
const knownTags = ref<CommerceTag[] | null>(null)
...
async function saveAssignment() {
  ...
  try {
    const assigned = await setTags.mutateAsync({ productUuid: product.uuid, tagUuids: selectedUuids.value })
    knownTags.value = assigned
    selectedUuids.value = assigned.map((t) => t.uuid)
    success('Tags updated', `${assigned.length} tag${assigned.length === 1 ? '' : 's'} set.`)
  } catch (e) {
    const err = toApiError(e)
...
<template>
  <div class="space-y-8">
    ...
    <section v-if="product" data-test="tag-assignment-section" class="space-y-3 border-t border-default pt-6">
      ...
      <UAlert
        ...
        description="Existing tag assignments aren't shown here yet — saving refreshes them for this session."
        data-test="tag-assignment-unknown"
      />
      ...
      <UButton
        v-if="canManage"
        size="xs"
        :loading="setTags.isLoading.value"
        label="Save tags"
