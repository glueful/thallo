...
Workspace SaaS billing for Thallo — platform plans and per-workspace subscriptions, as a removable capability pack.

Installed with Thallo as `glueful/thallo-subscriptions` (a capability pack required by
`glueful/thallo-core` at the same version); switched on and off through the admin's capabilities.
